...
@section('page_title', 'Analytical Result of Outgoing Shipment Product By Vessel')
        ...
        <table class="min-w-full bg-white border border-gray-200 rounded-lg">
            <thead class="bg-gray-100 text-gray-700 text-sm">
                <tr>
                    ...
                    <th class="px-4 py-2 border-b text-left">Vessel / Destination</th>
                ...
                @forelse($headers as $index => $doc)
                    <tr class="{{ $index % 2 ? 'bg-gray-50' : 'bg-white' }}">
                        ...
                        <td class="px-4 py-2 border-b">
                            <div class="text-xs text-gray-600">{{ $doc->vessel_name ?? '-' }}</div>
                            <div class="text-xs text-gray-400">{{ $doc->destination ?? ($doc->shipper ?? '-') }}</div>
                            <div class="text-xs text-gray-400">{{ optional($doc->sampling_date)->format('Y-m-d') ?? '-' }}</div>
                        </td>
                            ...
                            <div class="flex justify-center gap-2">
                                @if (empty($doc->prepared_status))
                                    @if (auth()->user()->roles === 'LEAD' || auth()->user()->roles === 'LEAD_QC')
                                        <form method="POST"
                                            action="{{ route('analytical-result-outgoing-shipment-product-by-vessel.approveReject', $doc->id) }}?status=Approved">
                                            @csrf
                                        ...
                                        <div x-data="{ open: false }">
                                            ...
                                            <div x-show="open" x-cloak
                                                ...
                                                <div @click.outside="open = false"
                                                    ...
                                                    <form method="POST"
                                                        action="{{ route('analytical-result-outgoing-shipment-product-by-vessel.approveReject', $doc->id) }}?status=Rejected">
                                                        @csrf
                        ...
                        <td class="px-4 py-2 border-b">
                            <div class="flex items-center gap-2 justify-between">
                                <a target="_blank"
                                    href="{{ route('analytical-result-outgoing-shipment-product-by-vessel.preview', $doc->id) }}?intention=preview"
                        ...
                        <td class="px-4 py-2 border-b text-center">
                            <a href="{{ route('analytical-result-outgoing-shipment-product-by-vessel.show', $doc->id) }}?intention=show"

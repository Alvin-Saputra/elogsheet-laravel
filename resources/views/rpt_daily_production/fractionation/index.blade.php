...
@section('content')
    @php
        ...
        // MODIFIED: 2024-03-02 - Dropdown for shift selection, keep 1 row per ticket
        $flatReports = $allItems->groupBy('id')->map(function ($rows) {
            $first = $rows->first();
            $shiftList = $rows->pluck('shift')->unique()->sort()->values();
            
            // Build shifts data array for dropdown and status display
            $shiftsData = [];
            foreach ($rows as $row) {
                                        ...
                                        <div x-show="showReject"
                                            ...
                                            <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-md border-t-4 border-yellow-500">
                                                ...
                                                <form method="POST"
                                                    ...
                                                    <input type="hidden" name="work_center" value="{{ $report->work_center }}">
                                                    <input type="hidden" name="approve_status" value="Rejected">
                                                    <label for="remark-{{ $report->id }}-{{ $shift }}" class="block mb-2">Reason for rejection:</label>
                                                    <textarea id="remark-{{ $report->id }}-{{ $shift }}" name="remark" class="w-full border rounded p-2" rows="3" required></textarea>
                                                    <div class="mt-6 flex justify-end gap-2">

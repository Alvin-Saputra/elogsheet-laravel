                ...
                <div class="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        <div class="flex">
                            ...
                            <span class="ml-2">:
                                {{ $displayDate ? \Carbon\Carbon::parse($displayDate)->format('d/m/Y') : '-' }}</span>
                        </div>
                        <div class="flex mt-1">
                    ...
                    @forelse($header->details as $d)
                        <tr>
                            ...
                            <td class="border px-2 py-1 text-center">
                                {{ in_array($d->status_ok, ['1', 'Y', 'OK']) ? '✓' : '' }}</td>
                            <td class="border px-2 py-1 text-center">
                                {{ in_array($d->status_ok, ['N', 'NO', 'NOT OK']) ? '✓' : '' }}</td>
            ...
            <div class="grid grid-cols-2 text-center text-xs mt-6 gap-4">
                <div class="space-y-1">
                    ...
                    <div class="font-medium">
                        {{ optional($header->preparedByUser)->fullname ?? ($header->prepared_by ?? '_________________') }}
                    </div>
                ...
                <div class="space-y-1">
                    ...
                    <div class="h-6"></div>
                    <div class="font-medium">
                        {{ optional($header->approvedByUser)->fullname ?? ($header->approved_by ?? '_________________') }}
                    </div>
                    <div class="text-xs text-gray-600">
        ...
        <div class="bg-white p-6 max-w-4xl mx-auto text-xs">
                ...
                <tr>
                    <td class="border p-2">
                        <div class="grid grid-cols-[110px_10px_1fr] gap-y-1">
                            ...
                            <div>:</div>
                            <div>
                                {{ $roa->date_received ? \Carbon\Carbon::parse($roa->date_received)->format('F d, Y') : '-' }}
                            </div>
                            ...
                            <div>:</div>
                            <div>
                                {{ $roa->date_reported ? \Carbon\Carbon::parse($roa->date_reported)->format('F d, Y') : '-' }}
                            </div>
                        </div>
                    </td>
            ...
            <div class="flex justify-end mt-12 mr-4">
                <div class="text-center w-48">
                    ...
                    <div class="font-bold">
                        {{ optional($roa->authorizedByUser)->fullname ?? ($roa->authorized_by ?? '_________________') }}
                    </div>
                </div>
            </div>

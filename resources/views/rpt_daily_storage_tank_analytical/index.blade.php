...
@section('content')
    <div class="bg-white p-6 rounded shadow-md">
        ...
        <div class="mb-4" x-data="{ bulkApproveAll: false, bulkRejectAll: false, bulkRemark: '' }">
            ...
            {{-- Bulk Approve Modal --}}
            <div x-show="bulkApproveAll" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50"
                x-cloak>
                <div class="bg-white p-6 rounded-lg shadow-xl">
                    ...
                    <div class="mt-6 flex justify-end gap-2">
                        ...
                        <form method="POST" action="{{ route('daily-storage-tank-analytical.bulk-approve') }}"
                            class="inline">
                            @csrf
            ...
            {{-- Bulk Reject Modal --}}
            <div x-show="bulkRejectAll" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50"
                x-cloak>
                <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
                    ...
                    <form method="POST" action="{{ route('daily-storage-tank-analytical.bulk-reject') }}">
                        ...
                        <textarea id="bulk-remark" name="remark" class="w-full border rounded p-2" rows="3" required
                            x-model="bulkRemark"></textarea>
                        <div class="mt-6 flex justify-end gap-2">
                            <button type="button" @click="bulkRejectAll = false"
                ...
                <tbody>
                    @forelse ($data as $item)
                        @php
                            ...
                            if ($item->transaction_date && $tanggal) {
                                $trxDate = \Carbon\Carbon::parse($item->transaction_date)->toDateString();

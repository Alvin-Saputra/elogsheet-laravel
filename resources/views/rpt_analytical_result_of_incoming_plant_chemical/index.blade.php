    ...
    <div class="bg-white p-6 rounded shadow-md">
        ...
        <div class="mb-4" x-data="{ bulkApproveAll: false, bulkRejectAll: false, bulkRemark: '' }">
            @if (auth()->user()->roles === 'LEAD_QC' || auth()->user()->roles === 'LEAD')
                <div class="flex gap-2">
                    <button type="button" @click="bulkApproveAll = true"
                        class="px-4 py-2 text-sm font-semibold rounded-lg {{ $headers->where('prepared_status', null)->count() > 0 ? 'bg-green-600 hover:bg-green-700 text-white' : 'bg-gray-300 text-gray-500 cursor-not-allowed' }}"
                        {{ $headers->where('prepared_status', null)->count() > 0 ? '' : 'disabled' }}>
                        Approve All
                    </button>
                    <button type="button" @click="bulkRejectAll = true"
                        class="px-4 py-2 text-sm font-semibold rounded-lg {{ $headers->where('prepared_status', null)->count() > 0 ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-gray-300 text-gray-500 cursor-not-allowed' }}"
                        {{ $headers->where('prepared_status', null)->count() > 0 ? '' : 'disabled' }}>
                        Reject All
                    </button>
                </div>
                @if ($headers->where('prepared_status', null)->count() === 0)
                    @if ($headers->count() === 0)
            ...
            @elseif (auth()->user()->roles === 'MGR_QC' || auth()->user()->roles === 'MGR' || auth()->user()->roles === 'ADM')
                <div class="flex gap-2">
                    <button type="button" @click="bulkApproveAll = true"
                        class="px-4 py-2 text-sm font-semibold rounded-lg {{ $headers->where('prepared_status', 'Approved')->where('approved_status', null)->count() > 0 ? 'bg-green-600 hover:bg-green-700 text-white' : 'bg-gray-300 text-gray-500 cursor-not-allowed' }}"
                        {{ $headers->where('prepared_status', 'Approved')->where('approved_status', null)->count() > 0 ? '' : 'disabled' }}>
                        Approve All
                    </button>
                    <button type="button" @click="bulkRejectAll = true"
                        class="px-4 py-2 text-sm font-semibold rounded-lg {{ $headers->where('prepared_status', 'Approved')->where('approved_status', null)->count() > 0 ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-gray-300 text-gray-500 cursor-not-allowed' }}"
                        {{ $headers->where('prepared_status', 'Approved')->where('approved_status', null)->count() > 0 ? '' : 'disabled' }}>
                        Reject All
                    </button>
                </div>
                @if ($headers->where('prepared_status', 'Approved')->where('approved_status', null)->count() === 0)
            ...
            <div x-show="bulkApproveAll" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" x-cloak>
                <div class="bg-white p-6 rounded-lg shadow-xl">
                    ...
                    <div class="mt-6 flex justify-end gap-2">
                        ...
                        <form method="POST" action="{{ route('analytical-result-incoming-plant-chemical-ingredient.bulk-approve') }}" class="inline">
                            @csrf
            ...
            <div x-show="bulkRejectAll" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" x-cloak>
                <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
                    ...
                    <form method="POST" action="{{ route('analytical-result-incoming-plant-chemical-ingredient.bulk-reject') }}">
                        @csrf
                        <input type="hidden" name="tanggal" value="{{ $selectedDate }}">

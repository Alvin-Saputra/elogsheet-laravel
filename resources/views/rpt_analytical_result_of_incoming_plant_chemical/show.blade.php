                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <div class="flex"><strong class="w-28">Date</strong>
                            <span
                                class="ml-2">{{ $displayDate ? \Carbon\Carbon::parse($displayDate)->format('d/m/Y') : '-' }}</span>
                        </div>
                    ...
                    <div>
                        ...
                        <div class="flex mt-1"><strong class="w-28">Exp Date</strong>
                            <span
                                class="ml-2">{{ $header->exp_date ? \Carbon\Carbon::parse($header->exp_date)->format('d/m/Y') : '-' }}</span>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    @forelse($header->details as $detail)
                        <tr>
                            ...
                            <td class="border px-2 py-1 text-center">
                                {{ in_array(strtoupper((string) $detail->status_ok), ['1', 'Y', 'OK']) ? '✓' : '' }}</td>
                            <td class="border px-2 py-1 text-center">
                                {{ in_array(strtoupper((string) $detail->status_ok), ['N', 'NO', 'NOT OK']) ? '✓' : '' }}</td>
            ...
            <div class="grid grid-cols-3 text-center text-xs mt-6 gap-4">
                <div>
                    ...
                    <div class="font-medium">
                        {{ optional($header->entriedByUser)->fullname ?? ($header->entry_by ?? '_________________') }}</div>
                ...
                <div>
                    ...
                    <div class="h-6"></div>
                    <div>{{ optional($header->preparedByUser)->fullname ?? ($header->prepared_by ?? '_________________') }}
                    </div>
                ...
                <div>
                    ...
                    <div class="h-6"></div>
                    <div>{{ optional($header->approvedByUser)->fullname ?? $header->approved_by ?? '_________________' }}</div>
                ...
                <tbody>
                    @forelse($coa?->details ?? [] as $cdetail)
                        <tr>
                            ...
                            <td class="border px-2 py-1 text-center">{{ $cdetail->actual_min }} -
                                {{ $cdetail->actual_max }}</td>
                            <td class="border px-2 py-1 text-center">{{ $cdetail->standard_min }} -
                                {{ $cdetail->standard_max }}</td>
